...
The forms of $G$ are in bijection with the elements of $H^1(\Gamma, \Aut(G))$.
...
\[ \ldots  H^1(\Gamma, G^{adj})  \rightarrow  H^1(\Gamma, \Aut(G)) \rightarrow  H^1(\Gamma, \mu_2) \rightarrow \ldots \]
We conclude we can characterize forms of $G$ by invariants in $H^1(\Gamma, \mu_2)$ and $H^1(\Gamma, G^{adj})$.

An element of $H^1(\Gamma, \mu_2)$ corresponds to a quadratic \'etale extension $E$ of $K$.
More precisely it is the $n+1$ torsion of $T_{E,\sigma}$.
We have an exact sequence:
\[ \ldots \rightarrow H^1(\Gamma,G) \rightarrow H^1(\Gamma, G^{adj}) \rightarrow H^2(\Gamma, \mu_{n+1}^\xi) \rightarrow \ldots. \]
A form of $G$ thus has an invariant in $H^2(\Gamma, mu_{n+1}^\xi)$.

$H^2(\Gamma, \mu_{n+1}^\xi)$ fits into the exact sequence:
\[ H^1(\Gamma, T_{E,\sigma}) \rightarrow H^2(\Gamma, mu_{n+1}^\xi) \rightarrow H^2(\Gamma, T_{E,\sigma}) \overset{[n+1]}\rightarrow H^2(\Gamma, T_{E,\sigma}). \]
...
$H^1(\Gamma, T_{E,\sigma}) = k^\times/N_{E/k}(E^\times)$ and that $H^2(\Gamma, T_{E,\sigma})$ is the kernel of the corestriction map from the Brauer group of $E$ to that of $E^\sigma$.
In particular we can now deduce that $H^2(\Gamma, \mu_{n+1}^\xi)$ corresponds to element a pair of an element of $\delta\in k^\times/(k^\times)^nN_{E/k}(E^\times)$ and a degree $n$ central simple algebra over $E$ in the $n$-torsion of the Brauer group which is only non-trivial at the places of $E$ over $K$ which split, and for places which split the algebra over one factor is isomorphic to the opposite algebra of that over the other.

Finally, we must consider the group $H^1(\Gamma,G)$. As $G$ is simply connect, this is only non-trivial at the real places of $K$.
In terms of real places, there are two cases to consider, when $E=\RR\times\RR$ then the $G$ is the automorphism group of a free module. As there are no forms of free modules, $H^1(\Gamma,G)$ is then trivial. If $E=\C$, then $G$ is the automorphism group of a Hermitian space and $H^1(\Gamma,G)$ classifies the Hermitian forms of the same discriminant as that defining $G$, over $\RR$ this means it classifies the signature.

TODO --- Clarify how twisting determines the order in which invariants should be considered.

Given a a collection of invariants, $E$, $A$, $\delta$, and signatures at real places of $K$ which ramify in $E$ we can construct the group $G$ as follows.
By assumption $A$ can be given an involution $\sigma_A$ of restricting to $\sigma$ on $E$.
